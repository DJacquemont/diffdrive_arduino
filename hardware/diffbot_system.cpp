  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  cfg_.left_servo_name = info_.hardware_parameters["left_servo_name"];
  ...
  servo_.setup(cfg_.left_servo_name);
...
std::vector<hardware_interface::CommandInterface> DiffDriveArduinoHardware::export_command_interfaces()
{
  ...
  command_interfaces.emplace_back(hardware_interface::CommandInterface(
    servo_.name, hardware_interface::HW_IF_VELOCITY, &servo_.cmd));
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & period)
{
  ...
  comms_.read_system_values(wheel_l_.pos, wheel_r_.pos, battery_level, duplo_cnt);
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  comms_.set_servo_values(servo_.cmd);
